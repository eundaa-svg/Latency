...
// Fullscreen ambient background for work list hover preview.
// Swap: add videoUrl to a work in portfolio.json to use video instead of CSS gradient.
...
export function ProjectBackground({ work, visible, reduced }: Props) {
  ...
  return (
    <motion.div className="absolute inset-0 pointer-events-none" animate={{ opacity }} transition={transition} aria-hidden>
      ...
      <div
        ...
        style={{ background: "radial-gradient(ellipse 80% 80% at 50% 50%, transparent 40%, rgba(5,5,5,0.55) 100%)" }}
      />
    </motion.div>
  );
}
...
function ProceduralBackground({ work, reduced }: { work: Work; reduced: boolean }) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ background: "var(--bg)" }}>
      <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse 70% 60% at 55% 45%, ${c}55 0%, ${c}18 45%, transparent 72%)` }} />
      <div className="absolute inset-0" style={{ background: `radial-gradient(ellipse 50% 50% at 20% 75%, ${c}28 0%, transparent 60%)` }} />
      {!reduced && (
        <motion.div
